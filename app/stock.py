...
def analyze_stock(stock, k_type=KType.DAY):
    ...
    if not prices:
        ...
        if len(matched_candlestick_patterns) != 0:
            for ma_pattern in ma_patterns:
                if ma_pattern.match(stock, prices):
                    print(f'Pattern {ma_pattern.name()} matched')
        ...
        if (len(matched_candlestick_patterns) != 0
            ...
            stock['patterns'].append(matched_candlestick_patterns)
            stock['patterns'].append(matched_ma_patterns)
            stock['patterns'].append(matched_volume_patterns)
